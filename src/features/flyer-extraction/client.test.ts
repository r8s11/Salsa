...
describe("extractEventFromFlyer", () => {
  ...
  it("returns a validated extraction response", async () => {
    ...
    await expect(
      extractEventFromFlyer(
        "https://project.supabase.co/storage/v1/object/public/event-flyers/user/submission-id/flyer.png"
      )
  ...
  it("translates function failures without exposing provider details", async () => {
    ...
    await expect(extractEventFromFlyer("https://project.supabase.co/flyer.png")).rejects.toThrow(
      "We couldn't read this flyer. Please try again."
    );
  });
