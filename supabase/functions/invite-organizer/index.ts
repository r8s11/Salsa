...
export type ServiceClient = {
  auth: {
    admin: {
      inviteUserByEmail: (email: string, options: { redirectTo: string; data: { display_name?: string } }) => Promise<AuthResult<{ user: User | null }>>;
      updateUserById: (id: string, attributes: { app_metadata: Record<string, unknown> }) => Promise<AuthResult<{ user: User | null }>>;
  ...
  from: (table: "profiles" | "audit_logs") => {
    ...
    delete: () => { eq: (column: string, value: string) => Promise<TableResult> };
  };
};
...
function runtimeDependencies(): InviteOrganizerDependencies {
  ...
  return {
    createCallerClient: (authorization) => createClient(supabaseUrl, anonKey, {
...
async function compensate(service: ServiceClient, userId: string, log: InviteOrganizerDependencies["log"]): Promise<void> {
...
export function createInviteOrganizerHandler(dependencies: InviteOrganizerDependencies) {
  return async (request: Request): Promise<Response> => {
    ...
    if (!isAllowedInviteRedirect(dependencies.redirectUrl)) {
      dependencies.log("invite-organizer invalid redirect configuration", { userId: caller.id });
    ...
    try {
      ...
      dependencies.log("invite-organizer service configuration unavailable", { userId: caller.id });
      return error("Invitation service is unavailable", 500);
    }
    ...
    try {
      ...
      if (!profile || profile.error) throw new Error("Could not provision organizer profile");
    ...
    const response: EmailInviteSuccess = { delivery: "email_invitation", userId: invited.id, email };
    return json(response, 200);
  };
}

if (import.meta.main) {
  serve((request) => createInviteOrganizerHandler(runtimeDependencies())(request));
}
