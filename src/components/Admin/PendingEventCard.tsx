...
export default function PendingEventCard({ event, onApprove, onReject, decision, error }: Props) {
  ...
  const rejectionFailed = confirmReject && error !== null;
  ...
  return (
    <article className="pending-event-card">
      ...
      <dl className="pending-event-card__metadata">
        <div>
          <dt>When</dt>
          <dd>{formatEventDateTime(event.event_date)}</dd>
        </div>
        <div>
          <dt>Price</dt>
          <dd>{isFree ? "Free" : `$${event.price_amount}`}</dd>
        </div>
        ...
        <div>
          <dt>Submitted</dt>
          <dd>{formatInNewYork(event.created_at, { year: "numeric", month: "long", day: "numeric" })}</dd>
        </div>
      </dl>
      ...
      {event.rsvp_link && (
        <a className="pending-event-card__rsvp" href={event.rsvp_link} target="_blank" rel="noopener noreferrer">
          View RSVP
        </a>
      )}

      {error && (
        <p className="pending-event-card__error" role="alert">
          {rejectionFailed ? `Rejection failed: ${error}` : `Approval failed: ${error}`}
        </p>
      )}
      ...
      <div className="pending-event-card__actions">
        {isDeciding ? (
          ...
          <>
            <button type="button" className="pending-event-card__reject-confirm" onClick={() => onReject(event.id)}>
          ...
          <>
            <button type="button" className="btn-primary" onClick={() => onApprove(event.id)}>Approve</button>
