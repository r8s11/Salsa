...
vi.mock("../../features/admin/hooks/useUserAuditLog", () => ({ useUserAuditLog }));

const { useResendOrganizerInvitation } = vi.hoisted(() => ({
  useResendOrganizerInvitation: vi.fn(),
}));
vi.mock("../../features/admin/hooks/useResendOrganizerInvitation", () => ({
  useResendOrganizerInvitation,
}));
...
describe("AdminUserDetailPage", () => {
  beforeEach(() => {
    ...
    vi.mocked(useResendOrganizerInvitation).mockReturnValue({ ...resendDefaultState });
  });

  it("offers no resend for an organizer who already accepted their invitation", () => {
    renderAt("organizer-1");
    expect(screen.queryByRole("button", { name: "Resend Invitation" })).not.toBeInTheDocument();
  });

  it("resends the pending invitation for the organizer being viewed", async () => {
    const resend = vi.fn();
    vi.mocked(useResendOrganizerInvitation).mockReturnValue({
      ...resendDefaultState,
      resend,
    });
    vi.mocked(useAdminUsers).mockReturnValue({
      ...defaultState,
      users: [{ ...organizer, email_confirmed_at: null }, guest],
    });
    renderAt("organizer-1");

    await userEvent.click(screen.getByRole("button", { name: "Resend Invitation" }));

    expect(resend).toHaveBeenCalledWith("organizer-1");
  });

  it("confirms the address the invitation went to", () => {
    vi.mocked(useResendOrganizerInvitation).mockReturnValue({
      ...resendDefaultState,
      sentTo: "maria@salsa.test",
    });
    vi.mocked(useAdminUsers).mockReturnValue({
      ...defaultState,
      users: [{ ...organizer, email_confirmed_at: null }, guest],
    ...
    expect(screen.getByRole("status")).toHaveTextContent("Invitation resent to maria@salsa.test");
  });

  it("surfaces a resend failure as an alert and keeps the button available", () => {
    vi.mocked(useResendOrganizerInvitation).mockReturnValue({
      ...resendDefaultState,
