...
vi.mock("../../features/events/components/EventFlyerField", () => ({
  default: function MockEventFlyerField() {
    return <input aria-label="Event flyer" type="file" />;
  },
}));
...
describe("AdminEventForm taxonomy selectors", () => {
  ...
  it("submits selected dance styles and attributes as taxonomy IDs", async () => {
    useActiveTaxonomyTerms.mockImplementation((category: string) => ({
      terms: category === "dance_style" ? [salsa] : [outdoor],
      isLoading: false,
      error: null,
    }));
    ...
    expect(onSubmit).toHaveBeenCalledWith(
      expect.objectContaining({ taxonomy_term_ids: ["salsa-id", "outdoor-id"] }),
      null
    );
  });
...
describe("AdminEventForm flyer replacement", () => {
  it("offers flyer replacement only for an existing event", () => {
    useActiveTaxonomyTerms.mockReturnValue({ terms: [], isLoading: false, error: null });
    render(
      <MemoryRouter>
        <AdminEventForm
          initial={initial}
  ...
  it("prevents duplicate saves and displays an async save failure", async () => {
    useActiveTaxonomyTerms.mockReturnValue({ terms: [], isLoading: false, error: null });
    ...
    rejectSubmit?.(new Error("Could not save event"));
    expect(await screen.findByRole("alert")).toHaveTextContent("Could not save event");
  });
});
