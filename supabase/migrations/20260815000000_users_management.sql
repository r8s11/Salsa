...
-- Invite a new user (creates auth.users row + profile).
-- Uses the Admin API pattern: generates a random temp password and sends an
-- invite email via Supabase Auth. Returns the new user's row for immediate UI use.
...
begin
  ...
  -- Insert into auth.users directly with a random temporary password.
  -- The user will receive a magic link / sign-up email from the invite.
  -- NOTE: In production, use supabase.auth.admin.inviteUser from a server-side
  -- function with the SERVICE_ROLE key. This RPC inserts directly into auth.users
  -- using the function owner's privileges (SECURITY DEFINER).
