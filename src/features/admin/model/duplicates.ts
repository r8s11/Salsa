...
export function detectDuplicates(
  ...
  for (const event of candidates) {
    ...
    // same-date
    if ((submissionData.event_date as string)?.split("T")[0] === event.event_date.split("T")[0]) {
      signals.push("same-date");
    }

    // similar-title
    if (
      (submissionData.title as string)?.trim().toLowerCase() === event.title.trim().toLowerCase()
    ) {
    ...
    // same-organizer
    if (
      (submissionData.host as string)?.trim().toLowerCase() === event.host?.trim().toLowerCase() &&
