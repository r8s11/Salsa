...
|-----------|---------|------|------|
| `supabase/manual/phase-11/` | 4 scripts | Settings table + audit trigger | Low — additive |
| `supabase/manual/phase-12/` | 4 scripts | Audit view, RPC, indexes, constraint, backfill | Low–Medium (backfill is optional) |
| `supabase/manual/phase-13/` | 4 scripts | Analytics view, RPCs, indexes, backfill | Low–Medium (backfill is optional) |
...
| **MUST FIX BEFORE CLOSE** | Grant `SELECT` on `audit_log_view` to `authenticated` — currently only the RPC is granted, but the view itself has no explicit grant. While the RPC is SECURITY DEFINER, the view should also be explicitly secured. | `supabase/manual/final-verification/05_fix_audit_view_grants.sql` | Low |
...
### Execution Order
1. Run `supabase/manual/final-verification/01_preflight_check.sql` (read-only) to verify prerequisites
2. Apply Phase 12 SQL: `supabase/manual/phase-12/001` → `002` → `003` → optional `004`
3. Apply Phase 13 SQL: `supabase/manual/phase-13/001` → `002` → `003` → optional `004`
4. Run `supabase/manual/final-verification/02_post_migration_verification.sql` (read-only) to confirm
5. Run `supabase/manual/final-verification/03_rls_security_check.sql` (read-only) to verify security
...
- See `supabase/manual/final-verification/README.md` for full details.
...
- Required SQL is prepared in `supabase/manual/final-verification/` with preflight and post-migration verification
