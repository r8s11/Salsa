...
export default function AccountPage() {
  const { user, role, signOut, clearDeletedAccount, updateEmail } = useAuth();
  ...
  const [emailChangedBanner] = useState(
    () => (location.state as { emailChanged?: boolean } | null)?.emailChanged === true
  );
  ...
  const handleEmailChangeSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (emailChangeStatus === "pending") return;
    ...
    if (error) {
      setEmailFormError(friendlyAuthError(error.message));
              ...
              <form onSubmit={(e) => void handleEmailChangeSubmit(e)} className="account-page__email-form" noValidate>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    ref={newEmailRef}
                    {...fieldErrorProps("account-new-email-error", emailFieldError)}
                  />
                  <FormFieldError id="account-new-email-error" message={emailFieldError} />
                </div>
                <Button
                  type="submit"
                  variant="secondary"
                  loading={emailChangeStatus === "pending"}
