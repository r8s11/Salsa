    ...
    auth: {
      admin: {
        inviteUserByEmail: async (email: string, options: unknown) => {
          ...
          return {
            data: {
              user: { id: "organizer-id", app_metadata: { provider: "email" } },
            },
            error: null,
          };
        },
...
Deno.test("answers browser preflight with CORS headers", async () => {
  ...
  const response = await createInviteOrganizerHandler(deps)(
    new Request("http://localhost/invite-organizer", {
      method: "OPTIONS",
      headers: {
        origin: "https://www.salsasegura.com",
...
Deno.test("rejects missing or malformed authorization", async () => {
  ...
  assertEquals(
    (await handler(request({ email: "person@example.com" }, ""))).status,
    401,
  );
  assertEquals(
    (await handler(request({ email: "person@example.com" }, "Basic abc")))
      .status,
    401,
  );
});
...
Deno.test("rejects authenticated callers without the admin app-metadata role", async () => {
  const { deps } = dependencies({
    createCallerClient: () => ({
      auth: {
        getUser: async () => ({
          data: {
            user: { id: "organizer-id", app_metadata: { role: "organizer" } },
          },
          error: null,
        }),
      },
  ...
  assertEquals(
    (await createInviteOrganizerHandler(deps)(
  ...
  assertEquals(calls, [
    {
      ...
      value: {
        email: "person@example.com",
        options: {
          redirectTo: "http://localhost:5173/auth/invite",
    ...
    },
    {
      ...
      value: {
        id: "organizer-id",
        update: { app_metadata: { provider: "email", role: "organizer" } },
      },
    },
    {
    ...
    },
    {
      ...
      value: {
        ...
        metadata: { email: "person@example.com", role: "organizer" },
      },
    },
  ]);
});
...
Deno.test("rolls back profile and Auth user after post-create failure without leaking secrets", async () => {
    ...
    createServiceClient: () =>
      ({
        auth: {
          admin: {
            inviteUserByEmail: async () => ({
              data: { user: { id: "organizer-id", app_metadata: {} } },
              error: null,
            }),
  ...
  assertEquals(response.status, 500);
  assertEquals(calls, [{ name: "delete:profiles", value: "organizer-id" }, {
    name: "delete-user",
    value: "organizer-id",
  }]);
...
Deno.test("rolls back when profile or audit provisioning fails", async () => {
  for (const failedStep of ["profile", "audit"]) {
    const { deps, calls } = dependencies({
      createServiceClient: () =>
        ({
          ...
          from: (table: string) => ({
            upsert: async () => ({
              error: failedStep === "profile"
                ? { message: "profile failed" }
                : null,
            }),
            insert: async () => ({
              error: failedStep === "audit"
    ...
    assertEquals(calls.slice(-2), [{
      ...
      value: "organizer-id",
    }, { name: "delete-user", value: "organizer-id" }]);
  }
});
