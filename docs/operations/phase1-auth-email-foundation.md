...
- One canonical auth callback route: `/auth/callback` (`src/features/auth/components/AuthCallback.tsx`) handles signup confirmation, password recovery, and future OAuth/magic-link returns.
...
- **Confirm signup**: paste from `supabase/templates/confirm-signup.md` (uses `{{ .ConfirmationURL }}`).
- **Reset password**: paste from `supabase/templates/reset-password.md` (uses `{{ .ConfirmationURL }}`).
- **Invite user**: paste from `supabase/templates/invite-organizer.md` (uses `{{ .ConfirmationURL }}`).
