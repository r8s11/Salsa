...
| V4 timezone | "open" | **OPEN and confirmed live** — `events.sql:7` declares `event_date timestamp with time zone`; `formatDateTimeForScheduleX` (`types/events.ts:102-110`) reads `getFullYear/getMonth/getDate/getHours` off a `new Date(...)` → renders in the **visitor's** timezone | `supabase/manual/legacy/events.sql`, `src/types/events.ts` |
