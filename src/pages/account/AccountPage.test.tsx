...
describe("AccountPage", () => {
  beforeEach(() => {
    ...
    mocks.auth.updateEmail.mockReset();
    mocks.auth.updateEmail.mockResolvedValue({ error: null });
  ...
  it("submits a valid new email and shows the dual-confirmation pending message", async () => {
    mocks.profile.profile = baseProfile();
  ...
  it("rejects an invalid new email address without calling updateEmail", async () => {
    mocks.profile.profile = baseProfile();
    const user = userEvent.setup();
    renderPage();
    ...
    expect(mocks.auth.updateEmail).not.toHaveBeenCalled();
  });

  it("rejects a resubmission of the current email address without calling updateEmail", async () => {
    mocks.profile.profile = baseProfile();
    const user = userEvent.setup();
    renderPage();

    await user.type(screen.getByLabelText("New email address"), "maria@example.com");
    await user.click(screen.getByRole("button", { name: "Change email" }));

    expect(screen.getByRole("alert")).toHaveTextContent(/already your current email/i);
    expect(mocks.auth.updateEmail).not.toHaveBeenCalled();
  });

  it("maps an updateEmail failure to safe copy and keeps the form usable", async () => {
    mocks.profile.profile = baseProfile();
    mocks.auth.updateEmail.mockResolvedValue({ error: new Error("already registered") });
    const user = userEvent.setup();
    renderPage();
  ...
  it("shows a confirmation banner after navigating back from a completed email-change link", () => {
    mocks.profile.profile = baseProfile();
    render(
      <MemoryRouter initialEntries={[{ pathname: "/account", state: { emailChanged: true } }]}>
        <Routes>
          <Route path="/account" element={<AccountPage />} />
        </Routes>
      </MemoryRouter>
    ...
    expect(screen.getByText("Your account email has been updated.")).toBeInTheDocument();
  });
