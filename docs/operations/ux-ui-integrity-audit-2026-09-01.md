...
- **Location:** `supabase/migrations/` (missing Phase-9/10/11 objects) vs `src/features/events/api/eventsRepo.ts:54-108` (embeds `event_taxonomy_terms(...)` in every read) and `SubmitEventPage`'s `public_event_suggestions_enabled()` RPC (defined in `supabase/reconcile-prod-schema.sql:1625`, `supabase/manual/phase-10/*`).
