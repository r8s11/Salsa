...
export interface SendAuthEmailDependencies {
  webhook: {
    verify(rawPayload: string, headers: Record<string, string>): unknown;
  };
  resend: {
    emails: {
      send(message: AuthEmail): Promise<{ data: unknown; error: unknown }>;
    };
  };
...
function template(
  ...
  switch (action) {
    case "invite": {
      ...
      const { subject, html } = organizerInvitationEmailContent({
        acceptUrl: url,
      });
      return { subject, html };
    }
    case "signup":
      return signupEmailContent(url);
    case "magiclink":
      return magicLinkEmailContent(url);
    case "recovery":
      return passwordRecoveryEmailContent(url);
  }
}
    ...
    if (payload.email_data.email_action_type === "email_change") {
      if (payload.email_data.token_hash_new) {
        ...
        messages.push({
          ...
          ...emailChangeCurrentAddressEmailContent(url, payload.user.new_email),
        });
      }
      if (payload.user.new_email) {
        ...
        messages.push({
          ...
          ...emailChangeNewAddressEmailContent(url),
        });
      }
      ...
      messages.push({
        from: deps.from,
        to: payload.user.email,
        ...template(payload.email_data.email_action_type, url),
      });
    }
