...
import type { City, DatabaseEvent } from "./types";
...
describe("selectRelatedEvents", () => {
  const current = makeEvent({
    id: "current",
    city: "boston" as City,
    event_date: "2026-08-24T12:00:00.000Z",
  });
  ...
  it("selects up to three later same-city events inside seven days", () => {
    const sameDay = makeEvent({
      id: "sameDay",
      city: "boston" as City,
      event_date: "2026-08-24T18:00:00.000Z",
    });
    const withinWeek = makeEvent({
      id: "withinWeek",
      city: "boston" as City,
      event_date: "2026-08-30T12:00:00.000Z",
    });
    const beyondWeek = makeEvent({
      id: "beyondWeek",
      city: "boston" as City,
      event_date: "2026-09-01T12:00:00.000Z",
    });
    const otherCity = makeEvent({
      id: "otherCity",
      city: "new-york-city" as City,
      event_date: "2026-08-25T12:00:00.000Z",
    });

    const selection = selectRelatedEvents(current, [
      current,
      sameDay,
      withinWeek,
      beyondWeek,
    ...
    expect(selection.events.map((event) => event.id)).toEqual([
      sameDay.id,
      withinWeek.id,
      beyondWeek.id,
    ]);
  ...
  it("backfills chronological later events after strict-window results", () => {
    ...
    const strict = makeEvent({
      id: "strict",
      city: "boston" as City,
      event_date: "2026-08-25T12:00:00.000Z",
    });
    const lateSecond = makeEvent({
      id: "lateSecond",
      city: "boston" as City,
      event_date: "2026-08-26T12:00:00.000Z",
    });
    const lateThird = makeEvent({
      id: "lateThird",
      city: "boston" as City,
      event_date: "2026-08-27T12:00:00.000Z",
    });
    const invalidDate = makeEvent({
      id: "invalidDate",
      city: "boston" as City,
      event_date: "invalid-date" as unknown as string,
    });
  ...
  it("only considers future events (strictly later than current)", () => {
    const sameTime = makeEvent({
      id: "sameTime",
      city: "boston" as City,
      event_date: "2026-08-24T12:00:00.000Z",
    });
    const oneSecondLater = makeEvent({
      id: "oneSecondLater",
      city: "boston" as City,
      event_date: "2026-08-24T12:00:01.000Z",
    });
  ...
  it("limits to three events", () => {
    const events = Array.from({ length: 5 }, (_, i) =>
      makeEvent({
        id: `event${i}`,
        city: "boston" as City,
        event_date: new Date(Date.parse(current.event_date) + (i + 1) * 3600000).toISOString(),
      })
    );
  ...
  it("hasStrictWindowEvents is false when no events within 7 days", () => {
    const justOutside = makeEvent({
      id: "justOutside",
      city: "boston" as City,
      event_date: new Date(Date.parse(current.event_date) + 8 * 24 * 3600000).toISOString(),
    }); // 8 days later
  ...
  it("handles invalid event_date gracefully", () => {
    const invalid = makeEvent({
      id: "invalid",
      city: "boston" as City,
      event_date: "not-a-date" as unknown as string,
    });
  ...
  it("backfills with beyond-window events when strict events are fewer than 3", () => {
    ...
    const strict = makeEvent({
      id: "strict",
      city: "boston" as City,
      event_date: new Date(currentTime + 24 * 3600000).toISOString(),
    }); // 1 day later
    // Two events beyond 7 days
    const beyondFirst = makeEvent({
      id: "beyondFirst",
      city: "boston" as City,
      event_date: new Date(currentTime + 8 * 24 * 3600000).toISOString(),
    }); // 8 days later
    const beyondSecond = makeEvent({
      id: "beyondSecond",
      city: "boston" as City,
      event_date: new Date(currentTime + 9 * 24 * 3600000).toISOString(),
    }); // 9 days later
