...
- **Manual production dashboard verification required (cannot be done from the repo — flagging per spec §3, not executing):**
  ...
  2. Confirm the **Confirm signup** email template in the hosted dashboard matches `supabase/templates/confirm-signup.md` and uses `{{ .ConfirmationURL }}` (already correct per that file).
...
- No hosted Supabase settings will be modified by this plan. `supabase/templates/confirm-signup.md`'s closing note ("navigates to `/`") will be corrected in Task 4 to describe the fixed behavior.
...
**Files:** `src/components/Auth/AuthCallback.tsx`, `src/components/Auth/AuthCallback.test.tsx`, `supabase/templates/confirm-signup.md`.
...
- Update `supabase/templates/confirm-signup.md`'s closing note (line 45) to describe the corrected behavior instead of documenting the bug as expected.
