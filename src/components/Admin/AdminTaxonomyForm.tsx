...
export default function AdminTaxonomyForm({
  ...
  initial: TaxonomyForm;
  submitLabel: string;
  usageCount?: number;
  isSaving?: boolean;
  onSubmit: (form: TaxonomyForm) => void;
  onCancel: () => void;
}) {
  const [form, setForm] = useState(initial);
  const [slugEdited, setSlugEdited] = useState(Boolean(initial.slug));
  const [errors, setErrors] = useState<Record<string, string>>({});
  const update = <K extends keyof TaxonomyForm>(key: K, value: TaxonomyForm[K]) =>
  ...
  return (
    <form className="admin-form" onSubmit={submit}>
      ...
      <fieldset className="admin-form__fieldset">
        ...
        <div className="admin-field">
          ...
          <select
            ...
            onChange={(e) => update("category", e.target.value as TaxonomyForm["category"])}
          >
            <option value="dance_style">Dance Style</option>
            <option value="event_attribute">Event Attribute</option>
          </select>
          {usageCount > 0 && (
            <p className="admin-form__helper">
              Category cannot change while this term has event usage.
            </p>
          )}
        </div>
        <div className="admin-field">
        ...
        <div className="admin-field">
          <label htmlFor="taxonomy-order">Display order</label>
          <input
            id="taxonomy-order"
            type="number"
            className="admin-input"
        ...
        <div className="admin-field">
          ...
          <select
            ...
            className="admin-select"
            value={form.status}
            onChange={(e) => update("status", e.target.value as TaxonomyForm["status"])}
          >
      ...
      <div className="admin-form__actions">
        ...
        <button className="admin-btn" disabled={isSaving}>
