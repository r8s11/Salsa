...
* **Step 5: Make timezone handling explicit in convert.ts**
  * **Target File(s):** `src/features/events/model/convert.ts`; read-only reference: `supabase/manual/legacy/events.sql`
  * **Objective:** Eliminate visitor-timezone-dependent rendering (V4) and fix the 2h-comment/4h-code mismatch.
  * **Instructions for Agent:** First inspect `supabase/manual/legacy/events.sql` to determine whether `event_date` is `timestamp` or `timestamptz`; record the answer in a comment at the top of `convert.ts`. Reimplement conversion using Temporal (already a global polyfill): if `timestamptz`, parse as `Temporal.Instant` → `toZonedDateTimeISO('America/New_York')`; if naive `timestamp`, parse as `Temporal.PlainDateTime` directly. Output format stays `"YYYY-MM-DD HH:mm"`. Define `const DEFAULT_DURATION_HOURS = 4` and correct the stale comment. `new Date(` must not appear in this file.
